...
def RGB(r, g, b):
    if OS_TYPE == 'nt': return r | (g << 8) | (b << 16)
    return (r << 16) | (g << 8) | b
...
class Synth:
    ...
    def tone(freqs, ms, vol=0.5, type='sine'):
        ...
        ns = int(Synth.SR * ms / 1000); b = bytearray()
        ...
        for i in range(ns):
            ...
            for f in freqs:
                if f > 0:
                    if type == 'noise': val += random.uniform(-1, 1)
                    else: val += math.sin(2*math.pi*f*(i/Synth.SR))
...
class Melody:
    ...
    def _compile_raw(bpm, notes):
        ...
        for i in clean_notes.split(','):
            ...
            d.extend(Synth.tone(freqs, dur*0.9))
            d.extend(Synth.tone([0], dur*0.1))
        return d
    ...
    def compile_to_memory(bpm, *tracks):
        ...
        max_len = max(len(t) for t in raw_tracks)
        arrays = []
        for t in raw_tracks:
...
class Physics:
    ...
    def resolve(b1, b2):
        ...
        if dist == 0 or dist > (b1.r + b2.r): return
...
class Mouse:
    ...
    def get_pos(self):
        if OS_TYPE == 'nt':
            ...
            pt = POINT()
            SYS['u32'].GetCursorPos(ctypes.byref(pt))
            SYS['u32'].ScreenToClient(self.engine.hw, ctypes.byref(pt))
            return pt.x, pt.y
        else:
...
class Win32Engine:
    def __init__(self, title, w, h, bg, fps=60, fullscreen=False):
        ...
        if fullscreen:
            ...
            self.w = SYS['u32'].GetSystemMetrics(0)
            self.h = SYS['u32'].GetSystemMetrics(1) 
            win_style = 0x80000000 | 0x10000000     
            start_x, start_y = 0, 0
        else:
            self.w, self.h = w, h
        ...
        # Убираем синее колесико: назначаем стандартный курсор
    ...
    def draw_text(self, x, y, text, c):
        ...
        SYS['g32'].SetBkMode(self.active_dc, 1) 
        SYS['g32'].TextOutA(self.active_dc, int(x), int(y), str(text).encode('ascii'), len(str(text)))

    def run(self):
        try:
            # 2.0: Используем perf_counter для микросекундной точности на Windows
            ...
            while True:
                ...
                # Гибридный слип (Engine 2.0 Precision Control)
                work_time = time.perf_counter() - now
                sleep_time = self.frame_delay - work_time
                
                if sleep_time > 0:
                    # Если времени много, спим грубо (отдаем процессор системе), но просыпаемся за 2 мс
                    if sleep_time > 0.002:
                        time.sleep(sleep_time - 0.002)
                    
                    # Оставшиеся доли миллисекунды ждем в пустом цикле (Spin-lock) для идеального FPS
